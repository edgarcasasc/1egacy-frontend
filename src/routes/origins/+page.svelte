<script>
  ...
  const todosLosLinajes = [
    { id: 'Garza', introduccion: 'El linaje Garza, arraigado en la nobleza del norte de España...', articulos: [{ titulo: 'Los Garza y la Fundación de Monterrey', url: '/blog/garza-1' }] },
    { id: 'Martínez', introduccion: 'Descubre la historia de los Martínez, un linaje de artesanos y pioneros...', articulos: [{ titulo: 'El Arte Perdido de la Forja', url: '/blog/martinez-1' }, { titulo: 'Isabel Martínez: La Matriarca', url: '/blog/martinez-2' }] },
    { id: 'Treviño', introduccion: 'De origen Cántabro, el apellido Treviño está ligado a la exploración...', articulos: [{ titulo: 'Diego de Treviño y el Camino Real', url: '/blog/trevino-1' }] },
  ...
  // --- VARIABLE REACTIVA: Filtra los linajes en tiempo real ---
  $: linajesFiltrados = terminoBusqueda === ''
    ? todosLosLinajes
    : todosLosLinajes.filter(linaje =>
        linaje.id.toLowerCase().includes(terminoBusqueda.toLowerCase())
      );
  ...
  onMount(() => {
    if (svgElement) dibujarConstelacion(todosLosLinajes);
  });
...
{#if linajesFiltrados.length > 0}
  ...
  {:else if terminoBusqueda !== ''}
    <div class="no-resultados-wrapper">
      ...
      <p>
        Somos un estudio en constante investigación. Déjanos tu correo y te
        notificaremos en cuanto tengamos información sobre este linaje.
      </p>
      <form class="formulario-interes">
