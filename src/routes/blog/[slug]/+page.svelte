<script>
  ...
  // --- IMPLEMENTACIÓN DE SCHEMA MARKUP CON E-E-A-T ---
  function createSchema(postData) {
    if (!postData) return {};

    // --- INICIO DE ACTUALIZACIÓN DE SCHEMA DE AUTOR ---
    const authorSchema = {
      '@type': 'Person',
      name: postData.author?.name || '1egacy Studio',
      // Agregamos la URL de la imagen del autor si existe
      ...(postData.author?.image && { image: postData.author.image }),
      // Agregamos el enlace social como "sameAs" para verificar identidad
      ...(postData.author?.socialLink && { sameAs: [postData.author.socialLink] })
    };
    // --- FIN DE ACTUALIZACIÓN DE SCHEMA DE AUTOR ---
      ...
      '@graph': [
        {
          ...
          mainEntityOfPage: {
            '@type': 'WebPage',
            '@id': `${baseUrl}/blog/${postData.slug?.current || ''}` // Usamos la baseUrl
          },
          headline: postData.title,
          description: postData.seoDescription || postData.subtitle,
          image: postData.mainImageUrl,
          author: authorSchema, // Usamos el schema de autor mejorado
          publisher: {
            '@type': 'Organization',
            name: '1egacy',
            logo: {
              '@type': 'ImageObject',
              url: `${baseUrl}/logo1egacy.svg` // Usamos la baseUrl
            }
          },
    ...
    if (validFaqsForTemplate.length > 0) {
      schema['@graph'].push({
        ...
        mainEntity: validFaqsForTemplate.map((faq) => ({
          ...
          acceptedAnswer: {
            '@type': 'Answer',
            // Convierte el 'block content' de Sanity a texto plano para el JSON-LD
...
<style>
  ...
  /* --- INICIO DE ESTILOS DE AUTOR AÑADIDOS --- */
  .author-bio-box {
    display: flex;
    align-items: center;
    gap: 2rem;
  ...
  /* --- FIN DE ESTILOS DE AUTOR AÑADIDOS --- */
</style>